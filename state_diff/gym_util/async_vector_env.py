...
# precedure to avoid borken pipe error in server
...
class AsyncVectorEnv(VectorEnv):
    ...
    def step_async(self, actions):
        ...
        if isinstance(actions, dict):
            for pipe, idx in zip(self.parent_pipes, np.arange(actions['action'].shape[0])):
                # Assuming the dictionary contains 'action' and 'state' keys
                np_action_dict = {'action': actions['action'][idx], 
                                  'action_pred': actions['action_pred'][idx]}
                pipe.send(("step", np_action_dict ))
        else:
    ...
    try:
        while True:
            ...
            if command == "reset":
                ...
                # if index == 0 or index == 1 or index ==6 or index ==8 or index ==9 or index ==7:
                #     print('here')
                ...
                # print(f"Worker-{index} sending result: {(reward, done), True}")
