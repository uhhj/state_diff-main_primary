...
class ReplayBuffer:
    ...
    def __init__(self, 
        ...
        def check_episode_length(value, episode_length):
            if isinstance(value, zarr.Group):
                for v in value.values():
                    check_episode_length(v, episode_length)
            else:
                assert value.shape[0] == episode_length, f"Inconsistent episode length for key: {key}"
