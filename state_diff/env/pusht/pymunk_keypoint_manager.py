...
class PymunkKeypointManager:
    def __init__(self, 
            ...
            local_allpoints_map: Dict[str, np.ndarray]=None,
        ...
        self.local_allpoints_map = local_allpoints_map
    ...
    def kwargs(self):
        return {
            ...
            'local_allpoints_map': self.local_allpoints_map,
    ...
    def create_from_pusht_env(cls, env, n_block_kps=9, n_agent_kps=3, seed=0, **kwargs):
        ...
        for name in ['block', 'agent']:
            ...
            if name == 'agent':
                self.agent = obj = self.add_circle(position=(256, 400), radius=15)
                n_kps = n_agent_kps
            elif name == 'block':
                self.block = obj = self.add_T((256, 300), 0)
                n_kps = n_block_kps
    ...
    @classmethod
    def create_from_pushut_env(cls, env, n_block_kps=9, n_Uobject_kps=13, n_agent_kps=3, seed=0, **kwargs):
        rng = np.random.default_rng(seed=seed)
        local_keypoint_map = dict()
        for name in ['block', 'U_object', 'agent']:
            self = env
            self.space = pymunk.Space()
            if name == 'agent':
                self.agent = obj = self.add_circle(position=(256, 400), radius=15)
                ...
                self.block = obj = self.add_T(position=(256, 300), angle=0)
                n_kps = n_block_kps
            elif name == 'U_object':
                self.U_object = obj = self.add_U(position=(256, 200), angle=0)  # adjust the parameters if necessary
                n_kps = n_Uobject_kps
    ...
    @classmethod
    def create_from_pushl_env(cls, env, n_L0object_kps=9, n_L1object_kps=10, n_agent_kps=3, seed=0, object_scale=30, debug=False, **kwargs):
        # Initialize a random number generator
        rng = np.random.default_rng(seed=seed)

        # Dictionary to store local keypoints for each object
        local_keypoint_map = dict()
        local_allpoints_map = dict()
        
        for name in ['L0_object', 'L1_object', 'agent']:
            self = env
            self.space = pymunk.Space()
            if name == 'agent':
                self.agent = obj = self.add_circle((256, 400), 15, 0)
                n_kps = n_agent_kps
            elif name == 'L0_object':
                self.block = obj = self.add_L0((256, 300), 0, scale=object_scale)
                n_kps = n_L0object_kps
            elif name == 'L1_object':
                self.block = obj = self.add_L1((256, 300), 0, scale=object_scale)
                n_kps = n_L1object_kps
            ...
            # Capture the rendered image and create a mask for the object
            img = np.uint8(pygame.surfarray.array3d(self.screen).transpose(1, 0, 2))
            obj_mask = (img != np.array([255, 255, 255], dtype=np.uint8)).any(axis=-1)

            # Transform the image of the object to local coordinates
            # tf_img_obj convert coordinates from the global space to the object's local coordinate space.
            tf_img_obj = cls.get_tf_img_obj(obj)
            
            # xy_img is a (512, 512, 2) array, where each element [i, j] is the (X, Y) pixel coordinates of that position in the image
            xy_img = np.moveaxis(np.array(np.indices((512, 512))), 0, -1)[:, :, ::-1]
            
            # local_coord_img is in the object's local coordinate system
            local_coord_img = tf_img_obj.inverse(xy_img.reshape(-1, 2)).reshape(xy_img.shape)
            obj_local_coords = local_coord_img[obj_mask]
            if debug:
                plt.figure(figsize=(10, 8))
                plt.scatter(obj_local_coords[:, 0], obj_local_coords[:, 1], s=1)  # s is the size of the point
                plt.xlabel('X Coordinate')
                plt.ylabel('Y Coordinate')
                plt.title('2D Visualization of obj_local_coords')
                plt.show()
            
            # Sample keypoints using furthest point sampling
            init_idx = rng.choice(len(obj_local_coords))
            obj_local_kps = farthest_point_sampling(obj_local_coords, n_kps, init_idx)
            
            # Add a small random shift to the keypoints
            small_shift = rng.uniform(0, 1, size=obj_local_kps.shape)
            obj_local_kps += small_shift            

            if debug:
                plt.figure(figsize=(10, 8))
                plt.scatter(obj_local_kps[:, 0], obj_local_kps[:, 1], s=2)  # s is the size of the point
                plt.xlabel('X Coordinate')
                plt.ylabel('Y Coordinate')
                plt.title('2D Visualization of obj_local_coords')
                plt.show()
                
            local_allpoints_map[name] = obj_local_coords
            # store sets of keypoints for different objects in their local coordinate systems
            local_keypoint_map[name] = obj_local_kps
        
        return cls(local_keypoint_map=local_keypoint_map, local_allpoints_map=local_allpoints_map, **kwargs)
...
def test():
    from state_diff.environment.push_t_env import PushTEnv
