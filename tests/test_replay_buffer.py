...
def test_real():
    ...
    dist_group = zarr.open(
        os.path.expanduser('~/dev/state_diff/data/pusht/pusht_cchi_v2.zarr'), 'r')
    ...
    # out_path = os.path.expanduser('~/dev/state_diff/data/pusht_cchi2_v2_replay.zarr')
    out_path = os.path.expanduser('~/dev/state_diff/data/test.zarr')
...
def test_pop():
    buff = ReplayBuffer.create_from_path(
        '/home/chengchi/dev/state_diff/data/pusht_cchi_v3_replay.zarr',
        mode='rw')
